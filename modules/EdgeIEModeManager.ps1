...
=======
###############################################################################
# Edge IE Mode Manager (Modular Edition)
###############################################################################
...
Import-Module (Join-Path $ModuleRoot "Language.psm1") -Force
Import-Module (Join-Path $ModuleRoot "Update.psm1") -Force
Import-Module (Join-Path $ModuleRoot "ADMX.psm1") -Force
Import-Module (Join-Path $ModuleRoot "IEManager.psm1") -Force
...
# Auto Update
Invoke-AutoUpdate -Config $Config

# Start GUI
Start-EdgeManagerUI -Config $Config
ea1f536 (Initial commit with modular structure)
